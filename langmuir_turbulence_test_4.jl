...
#grid = RegularRectilinearGrid(size=(128, 128, 128), extent=(128, 128, 128))
...
grid = VerticallyStretchedRectilinearGrid(size = (128, 128, 128),
                                          x = (0, 128),
                                          y = (0, 128),
...
#coriolis = FPlane(f=0.175e-4) # s⁻¹i
coriolis = FPlane(f=0) # s⁻¹f=0.175e-4
...
simulation.callbacks[:progress] = Callback(print_progress, IterationInterval(60))
...
#eddy_diffusivity = (; κₑ = model.diffusivity_fields.κₑ.T)


#fields_to_output = merge(model.velocities, model.tracers, eddy_viscosity,eddy_diffusivity)

#simulation.output_writers[:fields] =
#    JLD2OutputWriter(model, fields_to_output,
#                     schedule = TimeInterval(output_interval),
#                     prefix = "/Users/garanaik/run/Oceananigans/modified_examples/langmuir_turbulence/modified_example/test_2/averages_Us_0_Lx128_Nx128_48hr_OI6hr_II1",
#                     force = true)
...
#T = AveragedField(model.tracers.T, dims=(1, 2))

#wu = AveragedField(w * u, dims=(1, 2))
#wv = AveragedField(w * v, dims=(1, 2))
#wt = AveragedField(w * model.tracers.T, dims=(1, 2))
#w2 = AveragedField(w * w, dims=(1, 2))
#w3 = AveragedField(w * w * w, dims=(1, 2))
#v2 = AveragedField(v * v, dims=(1, 2))
#u2 = AveragedField(u * u, dims=(1, 2))
#uv = AveragedField(u * v, dims=(1, 2))

#avg_output=merge(u=U, v=V, T=T, wu=wu, wv=wv, wt=wt,w2=w2,w3=w3,v2=v2,u2=u2,uv=uv)
#simulation.output_writers[:averages] =
#    JLD2OutputWriter(model, (u=U, v=V, T=T, wu=wu, wv=wv, wt=wt,w2=w2,w3=w3,v2=v2,u2=u2,uv=uv),
#                     schedule = AveragedTimeInterval(output_interval, window=2minutes),
#                     prefix = "/Users/garanaik/run/Oceananigans/modified_examples/langmuir_turbulence/modified_example/langmuir_turbulence_averages_1_Us_0Lx128_Nx128_48hr_OI1hr_II1",
#                     force = true)
...
simulation.output_writers[:fields] =NetCDFOutputWriter(model,fields_to_output,
                       filepath = "/Users/garanaik/run/Oceananigans/modified_examples/langmuir_turbulence/modified_example/test_2/T0WindLat3_f0_fields.nc",
...
m_fluxes = LESbrary.TurbulenceStatistics.subfilter_momentum_fluxes(model)
t_fluxes = LESbrary.TurbulenceStatistics.subfilter_tracer_fluxes(model)
#www=AveragedField(w*w * T, dims=(1, 2))
subfilter_fluxes=merge(m_fluxes,t_fluxes)

using NCDatasets
using NetCDF
using Oceananigans.OutputWriters: NetCDFOutputWriter

simulation.output_writers[:averages] =NetCDFOutputWriter(model,merge(turbulence_statistics,subfilter_fluxes),
                     schedule = AveragedTimeInterval(output_interval,window=1minute),
                     filepath = "/Users/garanaik/run/Oceananigans/modified_examples/langmuir_turbulence/modified_example/test_2/T0WindLat3_f0.nc",
                     mode = "c")

#simulation.output_writers[:statistics] =
#                              JLD2OutputWriter(model,merge(turbulence_statistics,subfilter_fluxes),           
#                              schedule = AveragedTimeInterval(output_interval,window=0minute),
#                              prefix = "/Users/garanaik/run/Oceananigans/modified_examples/langmuir_turbulence/modified_example/test_2/T1WindUs1",
#                              force = true)
#
...
k = searchsortedfirst(grid.zᵃᵃᶠ[:], -2)
nothing # hide
...
anim = @animate for (i, iter) in enumerate(iterations)
    ...
    B_snapshot = averages_file["timeseries/T/$iter"][1, 1, :]
    ...
    wlims, wlevels = nice_divergent_levels(w, 0.01)
...
mp4(anim, "/Users/garanaik/run/Oceananigans/modified_examples/langmuir_turbulence/modified_example/test_2/averages_Us_0_Lx128_Nx128_48hr_OI6hr_II1.mp4", fps = 8) # hide
